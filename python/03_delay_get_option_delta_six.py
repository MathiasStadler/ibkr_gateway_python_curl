...
FILTER_DELTA = True          # True = nur Deltas -0.50..-0.10, False = alle (zum Testen)
...
def secdefInfo(conid, month, strike, right="P", exchange="SMART"):
    ...
    for contract in info_request.json():
        # Das 'right'-Feld aus der API übernehmen (falls vorhanden)
        ...
        contractDetails = {
            ...
            "right": contract_right          # Wichtig für Diagnose
        }
...
def writeResult(contracts_list):
    ...
    # 1. Filter nach Optionstyp (nur Puts)
    if FORCE_PUT_ONLY:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python script.py <TICKER> [MONTHS]")
        sys.exit(1)
    ticker = sys.argv[1].upper()
    num_months = int(sys.argv[2]) if len(sys.argv) >= 3 else 4
    logging.info(f"Ticker: {ticker}, months: {num_months}, PUT only: {FORCE_PUT_ONLY}, Delta filter: {FILTER_DELTA}")
    try:
