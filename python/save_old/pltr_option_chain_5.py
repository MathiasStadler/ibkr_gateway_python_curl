"""
Abruf von Put-Optionen für Palantir (PLTR) über die IBKR Web API.
...
# ------------------------------------------------------------
# 2. Hilfsfunktion: Nächste 3 Verfallszyklen ermitteln
# ------------------------------------------------------------
def get_next_expiry_months(months: List[str]) -> List[str]:
    ...
    for m in months:
        try:
            ...
            # Vereinfachung: Verfallstag ist der 15. des Monats (für Vergleich ausreichend)
...
# ------------------------------------------------------------
# 3. Hauptfunktion: Nur Puts mit Delta im Bereich [-0,50, -0,25]
# ------------------------------------------------------------
def get_filtered_puts(symbol: str, max_results: int = 30) -> List[Dict[str, Any]]:
    ...
    # Schritt 2: Nächste 3 Zyklen auswählen
    expiries = get_next_expiry_months(raw_months)
    print(f"📅 Nächste Verfallzyklen: {expiries}")

    # Schritt 3: Alle Put-Kontrakte sammeln (ohne Marktdaten)
    put_candidates = []
    ...
    # Schritt 4: Snapshot mit Delta (Feld 7314) abrufen
    final_puts = []
    batch_size = 90
                ...
                if delta_str:
                    try:
                        ...
                        if -0.50 <= delta <= -0.25:
                            ...
                            # Früher Stop, wenn max_results erreicht
...
# ------------------------------------------------------------
# 4. Ausführung (Test mit maximal 30 Puts)
# ------------------------------------------------------------
