...
# FILTER_DELTA = True          # True = nur Deltas -0.50..-0.10, False = alle (Test)
# FORCE_PUT_ONLY = True        # Entfernt alle Nicht-PUTs aus der CSV

FILTER_DELTA = False          # True = nur Deltas -0.50..-0.10, False = alle (Test)
FORCE_PUT_ONLY = True        # Entfernt alle Nicht-PUTs aus der CSV
...
def find_next_available_month(months, current_date):
    """Find the next available month from the provided month list."""
    if not months:
        return None
    ...
    try:
        ...
        # Convert month list to comparable format
        future_months = []
        for month in months:
            # Parse the YYYYMM string
            if len(month) != 6:
                continue
            year = int(month[:4])
            month_num = int(month[4:6])
            
            # Only consider months in the future
            if year > current_year or (year == current_year and month_num >= current_month):
                future_months.append((year, month_num, month))
        
        if not future_months:
            return None
            
        # Sort by year and month and take the first one
        future_months.sort(key=lambda x: (x[0], x[1]))
        return future_months[0][2]  # Return the month string
    
    except Exception as e:
        logging.error(f"Error finding next available month: {e}")
        return None

def secdefSearch(symbol):
    """Search for the underlying and return months in YYYYMM format."""
...
def write_debug_log(contracts_list, filename="option_debug.log"):
    """Write all contracts (with all current fields) to a JSON Lines file."""
...
if __name__ == "__main__":
    ...
    logging.info(f"Processing ticker: {ticker}, looking for next available months")
    ...
    nearest_month = find_next_available_month(months, current_date)
    if not nearest_month:
        logging.error(f"No future months found for {ticker}")
        sys.exit(1)
    
    # Get the position of the nearest month and select the next 'num_months' months
    try:
        month_index = months.index(nearest_month)
        # Select the nearest month and the following 'num_months' - 1 months
        sorted_months = months[month_index:month_index + num_months]
        logging.info(f"Nearest month found: {nearest_month}")
        logging.info(f"Processing months: {sorted_months}")
    except ValueError:
        logging.error(f"Nearest month {nearest_month} not found in month list")
        sys.exit(1)
