...
def secdefSearch(symbol, listingExchange):
    ...
    # Initialisiere mit None, um Fehler zu vermeiden
    underConid = None
    months = []

    for contract in search_request.json():
        if contract.get("description") == listingExchange:
            underConid = contract.get("conid")
    ...
    # Fehlerbehandlung, falls nichts gefunden wurde
    if underConid is None:
        raise ValueError(f"Contract with description '{listingExchange}' not found for symbol {symbol}")
    if not months:
        raise ValueError(f"No option months found for {symbol} on {listingExchange}")
